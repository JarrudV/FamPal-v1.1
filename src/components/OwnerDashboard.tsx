...
export default function OwnerDashboard({ place, userId, userEmail, onClose, onContentUpdated }: OwnerDashboardProps) {
  ...
  const renderSection = (section: (typeof template.sections)[0]) => {
    ...
    switch (section.type) {
      case 'text':
        return (
          <div key={section.key}>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{section.label}</label>
            {section.description && <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">{section.description}</p>}
            <input
              type="text"
              value={(content[section.key] as string) || ''}
              onChange={e => updateField(section.key, e.target.value as any)}
              className="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
        );
        ...
        return (
          <div key={section.key}>
            ...
            <textarea
              ...
              onChange={e => updateField(section.key, e.target.value as any)}
              rows={3}
              className="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
            />
          </div>
        );
      ...
      case 'boolean':
        return (
          <div key={section.key} className="flex items-center justify-between py-2">
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{section.label}</label>
            <button
              type="button"
              onClick={() => updateField(section.key, !content[section.key] as any)}
              className={`relative inline-flex h-6 w-11 rounded-full transition-colors ${content[section.key] ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'}`}
            >
            ...
            <div className="space-y-2">
              {(['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday', 'publicHolidays'] as const).map(day => (
                <div key={day} className="flex items-center gap-2">
                  <span className="w-28 text-xs text-gray-600 dark:text-gray-400 capitalize">{day === 'publicHolidays' ? 'Public Hols' : day}</span>
                  <input
                    type="text"
                    value={content.operatingHours?.[day] || ''}
                    onChange={e => updateHours(day, e.target.value)}
                    placeholder="e.g. 08:00 - 17:00"
                    className="flex-1 px-2 py-1.5 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-xs focus:ring-1 focus:ring-blue-500"
                  />
                </div>
              ))}
            </div>
          ...
          <div key={section.key}>
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {((content[section.key] as string[]) || []).map((item, i) => (
                <span key={i} className="inline-flex items-center gap-1 px-2.5 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 text-xs rounded-full">
                  {item}
                  <button
                    type="button"
                    onClick={() => {
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
            </div>
          </div>
        );
          ...
          <div key={section.key}>
            ...
            <div className="space-y-3">
              {(content.specialOffers || []).map(offer => (
                <div key={offer.id} className="p-3 border border-gray-200 dark:border-gray-700 rounded-lg space-y-2">
                  <div className="flex items-center justify-between">
                    <input
                      ...
                      className="flex-1 px-2 py-1.5 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-xs"
                    />
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full px-2 py-1.5 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-xs resize-none"
                  />
                  <div className="flex gap-2">
                    <input type="date" value={offer.validFrom || ''} onChange={e => updateOffer(offer.id, 'validFrom', e.target.value)} className="flex-1 px-2 py-1 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-xs text-gray-900 dark:text-white" />
                    <input type="date" value={offer.validUntil || ''} onChange={e => updateOffer(offer.id, 'validUntil', e.target.value)} className="flex-1 px-2 py-1 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-xs text-gray-900 dark:text-white" />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div key={section.key}>
            ...
            <div className="space-y-3">
              {(content.events || []).map(event => (
                <div key={event.id} className="p-3 border border-gray-200 dark:border-gray-700 rounded-lg space-y-2">
                  <div className="flex items-center justify-between">
                    <input
                      type="text"
                      value={event.title}
                  ...
                  />
                  <div className="flex gap-2">
                    <input type="date" value={event.date} onChange={e => updateEvent(event.id, 'date', e.target.value)} className="flex-1 px-2 py-1 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-xs text-gray-900 dark:text-white" />
                    <input type="time" value={event.time || ''} onChange={e => updateEvent(event.id, 'time', e.target.value)} className="flex-1 px-2 py-1 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-xs text-gray-900 dark:text-white" />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-2">
              {(['facebook', 'instagram', 'tiktok', 'website'] as const).map(platform => (
                <div key={platform} className="flex items-center gap-2">
                  <span className="w-20 text-xs text-gray-600 dark:text-gray-400 capitalize">{platform}</span>
                  <input
                    type="url"
      ...
      case 'photos':
        return (
          <div key={section.key}>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{section.label}</label>
            <p className="text-xs text-gray-500 dark:text-gray-400">Photo uploads coming soon. For now, contact us to add photos.</p>
          </div>
        );
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-white dark:bg-gray-900 rounded-t-2xl sm:rounded-2xl shadow-2xl" style={{ paddingBottom: 'calc(5rem + env(safe-area-inset-bottom, 0px))' }}>
        <div className="sticky top-0 z-10 bg-white dark:bg-gray-900 px-6 pt-5 pb-3 border-b border-gray-100 dark:border-gray-800">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">Manage Place</h2>
        ...
        <div className="px-6 py-4 space-y-5">
          {loadFailed && (
            <div className="p-3 bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 text-sm rounded-lg">Could not load latest data from server. Showing cached data — saving will overwrite.</div>
          )}
          {error && (
            <div className="p-3 bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300 text-sm rounded-lg">{error}</div>
          )}
          {saved && (
        ...
        {showUpgrade && (
          <div className="absolute inset-0 bg-white dark:bg-gray-900 z-20 overflow-y-auto rounded-2xl">
            <div className="px-6 pt-5 pb-3">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">Upgrade to Business Pro</h3>
                <button onClick={() => setShowUpgrade(false)} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-500">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                </button>
              </div>

              <div className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-xl p-5 mb-4">
                <div className="text-center mb-4">
                  <span className="text-3xl font-extrabold text-gray-900 dark:text-white">{BUSINESS_PRO_PRICE.label}</span>
                </div>
