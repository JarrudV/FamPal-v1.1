...
- **Replit session (2026-02-08)**: Fixed category cross-contamination (wine farms appearing in coffee searches). Tightened all 7 explore intent definitions in exploreIntentConfig.ts with stricter queries, keywords, and exclusions. Fixed derivePlaceCapabilities: wine farm detection uses name+description only (not address), coffee keyword gated to exclude wine farms, removed incorrect wine_farm/servesFood conflict. Added smart override logic in shouldKeepByIntent for wine farms with restaurants and parks with concessions to prevent over-filtering. DashboardNetflix.tsx completed. Increased free tier limits (memories 10→15, AI 10→15, circles 2→5), fixed circles entitlement bug.
