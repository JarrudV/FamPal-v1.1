...
- `lib/placeCache.ts` - Shared Firestore place detail cache (7-day TTL)
- `src/services/gamification.ts` - Explorer gamification system (points, levels, badges)
- `components/ExplorerLevel.tsx` - Explorer level UI component for Profile page
...
- `components/AdminReports.tsx` - Admin moderation view for community reports
- `src/services/communityReports.ts` - Community reports pipeline with trust scoring
...
- **Replit session (2026-02-17d)**: Community reports pipeline (src/services/communityReports.ts) with trust-scored signals, weighted voting (helpfulVotes boost, moderatorVerified 2x), confidence thresholds. Admin moderation view (components/AdminReports.tsx) gated by VITE_ADMIN_UIDS, accessible from Profile. "Family Verdict" section in VenueProfile shows aggregated community confirmations. Preference injection into Places API queries (food/activity preferences mapped to search boosters). Mobile touch targets increased to 44px (w-11 h-11). Notes/visited indicators on PlaceCard favorites.
- **Replit session (2026-02-17e)**: Firestore place detail cache (lib/placeCache.ts) — shared `placeCache` collection stores full PlaceDetails with 7-day TTL and version tagging. getPlaceDetails now checks: 1) localStorage (60min) → 2) Firestore (7 days) → 3) Google API (last resort). Writes back to both caches after API fetch. Saves ~$0.02–0.04 per repeat detail view. Admin UID auto-detection for unlimited AI credits. Core queries reduced from 3→2, optional capped at 1, total API calls per search ~3 (down from ~27). Search field mask moved to Essentials tier. Cache TTLs extended (places 60min, intents 30min). Enrichment batch 10 places, concurrency 2.
- **Replit session (2026-02-17f)**: Community-first data strategy — places with community reports/data now cached permanently in Firestore (no expiry), Google-only places expire after 90 days. `hasCommunityData` flag set when any community report, accessibility report, or family facility report is submitted. Firestore cache version bumped to v2 with `hasCommunityData` and `lastGoogleRefresh` fields. Updated firestore.rules for new placeCache fields. Goal: community-generated content becomes primary data source, Google Places API becomes fallback for gaps.
- **Replit session (2026-02-17g)**: FamPals Explorer gamification system (src/services/gamification.ts). Points awarded for contributions: community report (15), accessibility report (10), family facilities report (10), mark visited (5), save memory (5), add notes (3), helpful vote received (2). 7 Explorer Levels: Newcomer → Explorer → Trailblazer → Pathfinder → Guide → Champion → Legend. 7 achievement badges (First Report, Seasoned Reporter, Accessibility Advocate, Family Champion, Popular Contributor, Adventurer, Memory Maker). ExplorerLevel UI component (components/ExplorerLevel.tsx) on Profile page shows level, progress bar, points, badges, and how-to-earn breakdown. Points auto-awarded via dynamic imports after each contribution action. Stored in Firestore at users/{uid}/gamification/profile.

## Data Strategy
- **Community-first model**: User-contributed data (accessibility, family facilities, reports, verdicts) is permanent and takes priority in display.
- **Google Places as fallback**: Google API data fills gaps where community data doesn't exist yet. Cached in Firestore for 90 days.
- **3-tier cache**: localStorage (60min) → Firestore shared cache (90 days / permanent) → Google Places API (last resort).
- **Data provenance**: `hasCommunityData` flag on cached places distinguishes user-enriched vs Google-only records.
- **Cost architecture**: Firestore reads ~5,000x cheaper than Google Places API calls. Community data grows over time, reducing Google dependency.
