...
exports.generateFamilyFacilitiesSummary = generateFamilyFacilitiesSummary;
exports.aggregateFamilyFacilitiesFromReports = aggregateFamilyFacilitiesFromReports;
const FEATURE_ORDER = [
    'playground',
    'baby_changing_table',
    'stroller_friendly',
...
function generateFamilyFacilitiesSummary(facilities = []) {
    const confirmed = facilities.filter((item) => item.value === true && item.confidence !== 'unknown');
    if (confirmed.length === 0) {
        return 'Family info not yet confirmed.';
    }
    const isTrue = (feature) => facilities.some((item) => item.feature === feature && item.value === true && item.confidence !== 'unknown');
    const parts = [];
    if (isTrue('playground'))
        parts.push('Playground available');
    if (isTrue('baby_changing_table'))
        parts.push('Baby changing table available');
    if (isTrue('stroller_friendly'))
        parts.push('Stroller-friendly access reported');
    const comfort = [];
    if (isTrue('high_chairs'))
        comfort.push('high chairs');
    if (isTrue('kids_menu'))
        comfort.push("kids' menu");
    if (comfort.length > 0)
        parts.push(`${comfort.join(' and ')} available`);
    const support = [];
    if (isTrue('family_restroom'))
        support.push('family restroom');
    if (isTrue('nursing_room'))
        support.push('nursing room');
    if (isTrue('child_friendly_space'))
        support.push('child-friendly space');
    if (support.length > 0)
...
function aggregateFamilyFacilitiesFromReports(reports) {
    ...
    const familyFacilities = FEATURE_ORDER.map((feature) => {
...
//# sourceMappingURL=familyFacilities.js.map